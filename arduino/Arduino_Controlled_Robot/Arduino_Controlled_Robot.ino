...
long num = 0;
char mod;
const long TURN_LEFT_TICK = 910;    //950 original/tested 4/26 to tune value  ((895))  
const long TURN_RIGHT_TICK = 873;   //761 original/tested 4/26 to tune value  ((862))

int numCommands[2] = {18,11};

char cmdArray1[18] = {'G', 'T', 'G', 'T', 'G','T','G','T','G','T','P','G','T','G','T','G','A','D'};//,'G','T','G','T','G'};
long numArray1[18] = { 26,   1,  33,   1,  58,  1, 29,  1, 57,  2,  0, 74,  1, 31,  1, 70,  0,  0};//,  7,  1, 30,  1, 26};
char modArray1[18] = {'Z', 'L', 'Z', 'R', 'Z','L','Z','L','Z','R','Z','Z','R','Z','R','Z','Z','Z'};//,'Z','R','Z','R','Z'};

char cmdArray2[11] = {'G','A','P','G','T','G','T','G','A','D','G'};
long numArray2[11] = {160,  0,  0,170,  1, 35,  1,170,  0,  0,180};
char modArray2[11] = {'Z','Z','Z','Z','L','Z','L','Z','Z','Z','Z'};
...
void loop() {
  // put your main code here, to run repeatedly:
   while (!Done)
   {
	   for (int j = 0;j<1;j++) //cycle through separate victim pickup routines
	   {
		for (int i = 0; i < numCommands[j];i++) //cycle through commands in the pickup routine
		{
		  //if(Serial.available() > 0)
		  //{
			  if (j==0) //perform first pickup
			  {
				  delay(20);
				  cmd = cmdArray1[i];
				  delay(20);
			  ...
			  else if (j==1) // perform second pickup
			  {
				  delay(20);
				  cmd = cmdArray2[i];
				  delay(20);
...
void alignRobot()
{
  ...
  {
//    if (distanceAvg_L < lowerThresh && distanceAvg_R < lowerThresh)
//    {
//      travelDistance_Enc(CONV_FACTOR/2); //move forward a short distance
//    }
